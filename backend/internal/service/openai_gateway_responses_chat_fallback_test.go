...
// 场景（OpenCode 线上故障回归）：Codex Desktop 发 gpt-5.6-sol + reasoning.effort
// xhigh/max，账号把模型映射成第三方上游自己的模型（mimo-v2.5），上游不认识扩展
// 档位。分组配置了 max_reasoning_effort=high 后（入口处应用），xhigh/max/ultracode
// 一律降为 high 转发；未配置的分组保持透传，由管理员决定是否设限。
func TestForwardResponses_ForceChatCompletionsCapsExtendedEffortByGroupPolicy(t *testing.T) {
	for _, tc := range []struct{ effort string }{{"xhigh"}, {"max"}, {"ultracode"}} {
		t.Run(tc.effort, func(t *testing.T) {
			gin.SetMode(gin.TestMode)

			raw := []byte(`{"model":"gpt-5.6-sol","input":"hello","stream":false,"reasoning":{"effort":"` + tc.effort + `"}}`)
			// 入口 handler 在 Forward 之前应用分组策略（openai_gateway_handler）。
			body, _ := ApplyOpenAIReasoningEffortPolicy(raw, "high", nil)
			...
			upstream := &httpUpstreamRecorder{resp: &http.Response{
				StatusCode: http.StatusOK,
				Header:     http.Header{"Content-Type": []string{"application/json"}, "x-request-id": []string{"rid_effort_cap"}},
				Body: io.NopCloser(strings.NewReader(
			...
			require.NoError(t, err)
			require.NotNil(t, result)
			require.Equal(t, "mimo-v2.5", gjson.GetBytes(upstream.lastBody, "model").String())
...
// 对照：分组未配置 ceiling 时保持透传（是否设限由管理员决定）。
func TestForwardResponses_ForceChatCompletionsPassthroughWithoutGroupPolicy(t *testing.T) {
	gin.SetMode(gin.TestMode)

	body := []byte(`{"model":"gpt-5.6-sol","input":"hello","stream":false,"reasoning":{"effort":"xhigh"}}`)
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/responses", bytes.NewReader(body))
	c.Request.Header.Set("Content-Type", "application/json")
	...
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"application/json"}, "x-request-id": []string{"rid_passthrough"}},
		Body: io.NopCloser(strings.NewReader(
